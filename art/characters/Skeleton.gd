...
func _physics_process(_delta):
	...
	if not (target_global_position == Vector3.ZERO):
		_velocity = follow(
			_velocity,
	...
	if (global_transform.origin - target_global_position).length_squared() < 0.1:
		stop_moving()
	
func _unhandled_input(event):
	if event.is_action_pressed("click") and move_spell_selected:
		...
		if not intersection.empty() and intersection.position.y < 0.1 and intersection.position.y > -0.1:
			target_global_position = intersection.position
